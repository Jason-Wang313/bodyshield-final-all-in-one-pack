"""Run the BodyShield release runtime smoke audit."""
...
from bodyshield.release_runtime_audit import (
    failed_release_runtime_rows,
    release_runtime_summary,
    write_release_runtime_audit_reports,
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--timeout-s", type=int, default=240, help="Timeout for the extracted pytest run.")
    parser.add_argument("--json", action="store_true", help="Print JSON summary instead of compact text.")
    args = parser.parse_args(argv)

    rows = write_release_runtime_audit_reports(args.root, timeout_s=args.timeout_s)
    ...
    if args.json:
        ...
        print(f"RELEASE_RUNTIME_AUDIT_STATUS={status}")
