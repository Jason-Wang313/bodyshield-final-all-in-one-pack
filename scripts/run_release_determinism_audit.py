"""Run the BodyShield release determinism audit."""
...
from bodyshield.release_determinism_audit import (
    ...
    release_determinism_summary,
    write_release_determinism_audit_reports,
)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--root", type=Path, default=ROOT, help="Full pack root.")
    parser.add_argument("--json", action="store_true", help="Print JSON summary instead of compact text.")
    args = parser.parse_args(argv)

    rows = write_release_determinism_audit_reports(args.root)
    ...
    if args.json:
        ...
        print(f"RELEASE_DETERMINISM_AUDIT_STATUS={status}")
