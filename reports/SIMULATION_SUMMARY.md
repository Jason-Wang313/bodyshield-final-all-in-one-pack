...
The readiness harness produced 2 rows, including 1 passing deterministic fixture smoke row(s), 1 external checkpoint spec row(s), and 1 missing checkpoint row(s). It validates the interface path only; it is not external trained-policy benchmark evidence.
